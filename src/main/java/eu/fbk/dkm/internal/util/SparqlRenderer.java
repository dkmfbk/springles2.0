...
public final class SparqlRenderer {
	 ...
	 * @uml.associationEnd  qualifier="namespace:java.lang.String java.lang.String"
	 */
    ...
    public static SparqlRenderer render(final Value value) {
        ...
        return new SparqlRenderer(Collections.singleton(new ValueConstant(value)),
                Collections.<Dataset>singleton(null), false, false);
    }
    ...
    public static SparqlRenderer render(final Iterable<? extends UpdateExpr> updates,
        ...
        return new SparqlRenderer(updates, datasets, true, false);
    }
    ...
    public void toStringBuilder(final StringBuilder builder) {
        ...
        for (final QueryModelNode node : this.nodes) {
            ...
            final Rendering rendering = new Rendering(node, dataset);
            if (this.headerRendered) {
                ...
                if (!rendering.namespaces.isEmpty()) {
                    for (final String namespace : Ordering.natural().sortedCopy(
                        ...
                        if ("bif".equals(prefix)
                                && "http://www.openlinksw.com/schema/sparql/extensions#" //
                                .equals(namespace)) {
                            continue; // do not emit Virtuoso bif: binding, as Virtuoso will
                                      // complain
                        }
    ...
    private final class Rendering implements QueryModelVisitor<RuntimeException> {
        ...
        private Rendering(final QueryModelNode node, @Nullable final
        		Dataset dataset) {
        		            ...
        		            if (node instanceof TupleExpr) {
        		                this.root = new
        		QueryRoot(Preconditions.checkNotNull((TupleExpr) node));
        		                emit(Query.create(this.root, this.dataset,
        		SparqlRenderer.this.forceSelect));
        		            } else {
        		            	this.root=null;
        		                emit(node);
        		            }
