...
public final class RDFParseOptions implements Cloneable
{
    ...
    {
        this(format, baseURI, valueFactory, RDFParseOptions.DEFAULT_PARSER_CONFIG);
    }

    public RDFParseOptions(final RDFFormat format, final String baseURI,
            final ValueFactory valueFactory, final ParserConfig parserConfig)
    {
        final ParserConfig config = MoreObjects.firstNonNull(parserConfig,
                RDFParseOptions.DEFAULT_PARSER_CONFIG);
    ...
    {
        final RDFParser parser = RDFParserRegistry.getInstance().get(this.format).get()
                .getParser();
    ...
    public int hashCode()
    {
        return Objects.hashCode(this.format, this.baseURI,
                System.identityHashCode(this.valueFactory), this.preserveBNodeIDs,
